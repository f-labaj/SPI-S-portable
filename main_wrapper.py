...
while True:
    ...
    # WARNING! This will overwrite any measurements currently stored in memory
        ...
        if filename != '':
            ...
            intensity_coeff_list = aux.load_measurements('./EXAMPLES/' + filename)
                    ...
                    while True:
                        ...
                        if BBB_event == "Exit" or BBB_event == sg.WIN_CLOSED:
                            ...
                            if len(pattern_list) != 0:
                                ...
                                aux.save_measurement_vector(intensity_vector, './EXAMPLES/intensity_' + str(values["-RESOLUTION-"]) + '_-' + str(values["-FACTOR-"]) + '-')
                                ...
                                aux.save_measurement_vector(intensity_vector, './EXAMPLES/norm_intensity_' + str(values["-RESOLUTION-"]) + '_-' + str(values["-FACTOR-"]) + '-')
                                aux.save_measurement_vector(intensity_vector_sum, './EXAMPLES/norm_sum_intensity_' + str(values["-RESOLUTION-"]) + '_-' + str(values["-FACTOR-"]) + '-')
                                ...
                                aux.save_measurements(intensity_coeff_list, './EXAMPLES/meas_' + str(values["-RESOLUTION-"]) + '_-' + str(values["-FACTOR-"]) + '-')
                                ...
                                aux.save_measurements(intensity_coeff_list_sum, './EXAMPLES/meas_sum_' + str(values["-RESOLUTION-"]) + '_-' + str(values["-FACTOR-"]) + '-')
                                
                                # DEBUG
                                # invert measured values
                                #intensity_vector_inv = np.invert(np.asarray(intensity_vector)).tolist()
                                
                                #for i in range(0, len(intensity_vector_inv), 3):
                                #	intensity_coeff_list_inv.append((2*intensity_vector_inv[i] - intensity_vector_inv[i+1] - intensity_vector_inv[i+2]) + np.sqrt(3)*1j*(intensity_vector_inv[i+1] - intensity_vector_inv[i+2]))

                                #aux.save_measurements(intensity_coeff_list_inv, './EXAMPLES/inv_meas_' + str(values["-RESOLUTION-"]) + '_-' + str(values["-FACTOR-"]) + '-')
